...
from nets.gan_cnn import CGANGenerator
...
class GANSampler:
    ...
    def __init__(self, sample_dataloader, config, **kwargs):
        ...
        self.gan_samples_saving_path = config.gan_samples_saving_path

        # same knobs as diffusion sampler
        self.AR = config.AR
    ...
    def sample(self):
        # build model
        self.G = CGANGenerator(
            z_dim = self.z_dim,
            base_channels = self.base_channels,
            out_channels = 2
        ).to(self.device)

        ckpt = torch.load(self.gan_model_saving_path, map_location = self.device)
        if isinstance(ckpt, dict) and "G" in ckpt:
            self.G.load_state_dict(ckpt["G"], strict = True)
        else:
            # allow loading only generator weights
            self.G.load_state_dict(ckpt, strict = True)
        ...
        fake_samples = []
        loop = tqdm(self.sample_dataloader, desc = "GAN Sampling", total = len(self.sample_dataloader))
        ...
        for idx, (past_batch, predict_batch, trend_batch, volatility_batch, liquidity_batch, imb_batch, _, predict_time_batch) in enumerate(loop):
            ...
            with torch.no_grad():
                samples = self.G(z, cond, F_dim = self.F, T_dim = self.predict_window)  # [B, 2, F, T]
        ...
        np.save(self.gan_samples_saving_path, fake_samples)
