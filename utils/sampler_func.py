...
from .util import from_flattened_numpy, to_flattened_numpy
...
def get_sampling_fn(config, sde, shape, eps, device):
    ...
    if sampler_name.lower() == 'ode':
        sampling_fn = get_ode_sampler(sde = sde,
                                      shape = shape,
                                      denoise = config.sampling_noise_removal,
                                      eps = eps,
                                      device = device)
        
    # Predictor-Corrector sampling. Predictor-only and Corrector-only samplers are special cases.
    elif sampler_name.lower() == 'pc':
        predictor = get_predictor(config.sampling_predictor.lower())
        corrector = get_corrector(config.sampling_corrector.lower())
...
class Predictor(abc.ABC):
    ...
    @abc.abstractmethod
    def update_fn(self, x, t, cond, guidance):
...
class EulerMaruyamaPredictor(Predictor):
    ...
    def update_fn(self, x, t, cond, guidance):
        ...
        drift, diffusion = self.rsde.sde(x, t, cond, guidance)
        x_mean = x + drift * dt
...
class ReverseDiffusionPredictor(Predictor):
    ...
    def update_fn(self, x, t, cond, guidance):
        f, G = self.rsde.discretize(x, t, cond, guidance)
        z = torch.randn_like(x)
        x_mean = x - f
...
class AncestralSamplingPredictor(Predictor):
    ...
    def vesde_update_fn(self, x, t, cond, guidance, enable_motion, enable_control):
        sde = self.sde
    ...
    def vpsde_update_fn(self, x, t, cond, guidance, enable_motion, enable_control):
    ...
    def update_fn(self, x, t, cond, guidance, enable_motion, enable_control):
        if isinstance(self.sde, sde_lib.VESDE):
            return self.vesde_update_fn(x, t, cond, guidance, enable_motion, enable_control)
        elif isinstance(self.sde, sde_lib.VPSDE):
            return self.vpsde_update_fn(x, t, cond, guidance, enable_motion, enable_control)
...
class LangevinCorrector(Corrector):
    ...
    def update_fn(self, x, t, cond, guidance):
        ...
        for i in range(n_steps):
            grad = guidance * score_fn(x, t, cond) + (1 - guidance) * score_fn(x, t, torch.zeros_like(cond))
            noise = torch.randn_like(x)
...
def shared_predictor_update_fn(x, t, cond, guidance, model, enable_motion, enable_control, sde, predictor, probability_flow, continuous):
    ...
    return predictor_obj.update_fn(x, t, cond, guidance, enable_motion, enable_control)


def shared_corrector_update_fn(x, t, cond, guidance, model, enable_motion, enable_control, sde, corrector, continuous, snr, n_steps):
    ...
    return corrector_obj.update_fn(x, t, cond, guidance, enable_motion, enable_control)
...
def get_pc_sampler(sde, shape, predictor, corrector, snr,
    ...
    def pc_sampler(model, cond, guidance, enable_motion, enable_control):
        ...
        with torch.no_grad():
            ...
            for i in range(sde.N):
                ...
                x, x_mean = corrector_update_fn(x, vec_t, cond, guidance, model, enable_motion, enable_control)
                x, x_mean = predictor_update_fn(x, vec_t, cond, guidance, model, enable_motion, enable_control)
            
            # sde.N * (n_steps + 1) is the denoising steps in total,
            # where sde.N is predictor steps, sde.N * n_steps is corrector steps
...
def get_ode_sampler(sde, shape, 
    ...
    def denoise_update_fn(model, x, cond, guidance):
        ...
        _, x = predictor_obj.update_fn(x, vec_eps, cond, guidance)
        return x

    def drift_fn(model, x, t, cond, guidance):
        """Get the drift function of the reverse-time SDE."""
        score_fn = get_score_fn(sde, model, continuous = False)
        rsde = sde.reverse(score_fn, probability_flow = True)
        return rsde.sde(x, t, cond, guidance)[0]

    def ode_sampler(model, cond, guidance, z = None):
        ...
        with torch.no_grad():
            ...
            def ode_func(t, x):
                ...
                drift = drift_fn(model, x, vec_t, cond, guidance)
            ...
            if denoise:
                x = denoise_update_fn(model, x, cond, guidance)
...
def get_dpm_solver_sampler(sde, shape, steps = 10, eps = 1e-3,
    ...
    def dpm_solver_sampler(model):
        ...
        with torch.no_grad():
            ...
            dpm_solver = DPM_Solver(noise_pred_fn, ns, algorithm_type = algorithm_type,
