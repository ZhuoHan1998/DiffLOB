...
class diff_S4(nn.Module):
    """
    Wrap Residual_group so that:
      forward(x, diffusion_steps, cond) -> same shape as x
    x:    [B, input_dim, 20, T]
    ...
    def __init__(
        ...
        self.levels = 20  # number of price levels or spatial dimension

        # ---- Flatten the level dimension into the channel dimension ----
        in_channels_flat = input_dim * self.levels          # flattened input channels
        cond_channels_flat = cond_dim * self.levels         # flattened conditional channels

        # Residual_group expects conditional channels as 2 * in_channels.
        # We compute in_channels_rg so that 2*in_channels_rg == cond_channels_flat.
        in_channels_rg = cond_channels_flat // 2
        
        # Project flattened input channels to residual channels
        self.in_proj = nn.Conv1d(in_channels_flat, res_channels, kernel_size=1)

        # Residual_group (output skip: [B, skip_channels, T])
        self.group = Residual_group(
            ...
            diffusion_step_embed_dim_out=diffusion_step_embed_dim_out,
            in_channels=in_channels_rg,                # ensures 2*in_channels matches flattened conditional channels
            s4_lmax=s4_lmax,
            s4_d_state=s4_d_state,
            s4_dropout=s4_dropout,
            s4_bidirectional=s4_bidirectional,
        ...
        # Project skip output back to flattened input channels
